...
/*
    Note on Trap vs Interrupt Gates (from OSDev.org):

    Trap and Interrupt gates are similar, and their descriptors
    are structurally the same, they differ only in the "type" field.
    The difference is that for interrupt gates, interrupts are
    automatically disabled upon entry and reenabled upon IRET
...
typedef union idt_desc_t {
    uint32_t val[2];
    struct {
        uint16_t offset_15_00;  // bits [15:0] of offset
        uint16_t seg_selector;  // segment selector
        uint8_t  reserved4;     // unused 8 bits [39:32]
        uint32_t reserved3 : 1; // 0 = interrupt, 1 = trap
        uint32_t reserved2 : 1; // part of gate type bits
        uint32_t reserved1 : 1; // part og gate type bits
        uint32_t size      : 1; // 1 = 32 bit intt or trap gates
        uint32_t reserved0 : 1; // storage segment: 0 for intt. and trap gates
        uint32_t dpl       : 2; // 0 = kernel, 3 = user
        uint32_t present   : 1; // 0 for unused interrupts, else 1
        uint16_t offset_31_16;  // bits [31:16] of offset
    } __attribute__ ((packed));
} idt_desc_t;
